...
def delete_load_balancer(load_balancer_arn_or_name, is_classic=False):
    ...
    if is_classic:
        ...
        if active_elb:
            mpe_utils.log_error(f"Cannot delete Load Balancer {load_balancer_arn_or_name} due to associated below active services.")
            if tg_details:
...
def main():
    ...
    # If a file is provided, read Load Balancers from file
    elif not os.path.isfile(input_param):
        ...
        for lb_id in lb_ids:
            if lb_id in all_lbs:
                ...
                mpe_utils.log_info(f"Load Balancer: {lb_id} | Type: {lb_type} | State: {lb_state}")
                deletable_lbs.append(lb_id)
            else:
                mpe_utils.log_error(f"Invalid Load Balancer from input file: {lb_id}")
