...
def describe_nat_gateways():
    """Fetches all NAT Gateways in the region."""
...
def check_route_tables(nat_gateway_id):
    """Checks if a NAT Gateway has any associated route tables."""
    ec2_client = boto3.client('ec2', region_name=AWS_REGION)
    try:
        ...
        mpe_utils.log_error(f"Failed to fetch route tables for NAT Gateway {nat_gateway_id}: {e}")
        return []

def wait_for_nat_deletion(nat_gateway_id):
    """Waits for the NAT Gateway to be fully deleted."""
    ec2_client = boto3.client('ec2', region_name=AWS_REGION)
...
def release_elastic_ip(allocation_id):
    """Releases the Elastic IP associated with the NAT Gateway."""
    ec2_client = boto3.client('ec2', region_name=AWS_REGION)
    try:
...
def delete_nat_gateway(nat_gateway_id):
    """Deletes a NAT Gateway if no route tables are associated with it."""
    ec2_client = boto3.client('ec2', region_name=AWS_REGION)
...
def main():
    """Handles input parameters, lists NAT Gateways, and optionally deletes unused ones."""
    parser = argparse.ArgumentParser(description="Manage AWS NAT Gateways")
    parser.add_argument('-i', '--input', required=True, help="Specify 'ALL' or a file containing NAT Gateway IDs")
    ...
    # If "ALL" is provided, fetch all NAT Gateways
    if input_param == "all":
        mpe_utils.log_info("Fetching all NAT Gateways...")
        ...
        for nat in all_nat_gateways:
            ...
            # Only add to deletable list if no associated route tables and not already deleted
          
               

    # If a file is provided, read NAT Gateway IDs from file
    elif os.path.isfile(input_param):
        with open(input_param, 'r') as file:
            nat_ids = [line.strip() for line in file.readlines()]

        all_nat_gateways = describe_nat_gateways()
        valid_nats = {nat['NatGatewayId']: nat for nat in all_nat_gateways}

        for nat_id in nat_ids:
            if nat_id in valid_nats:
                state = valid_nats[nat_id]['State']
                allocation_id = valid_nats[nat_id].get('NatGatewayAddresses', [{}])[0].get('AllocationId')
        ...
        mpe_utils.log_error(f"Error: File '{input_param}' does not exist.")
        return

    # If action is DELETE, attempt to delete NAT Gateways
    if action == "DELETE":
